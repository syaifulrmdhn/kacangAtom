...
class StatusIcon extends HTMLElement
  ...
  initialize: (@terminalView) ->
    @classList.add 'vk-terminal-status-icon'
  ...
  setupTooltip: ->
    ...
    onMouseEnter = (event) =>
      return if event.detail is 'vk-terminal'
      @updateTooltip()
  ...
  updateTooltip: ->
    ...
    @dispatchEvent(new CustomEvent('mouseenter', bubbles: true, detail: 'vk-terminal'))
...
module.exports = document.registerElement('vk-terminal-status-icon', prototype: StatusIcon.prototype, extends: 'li')
